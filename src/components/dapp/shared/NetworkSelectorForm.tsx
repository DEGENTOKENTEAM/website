    ...
    return (
        <RadioGroup value={selectedChain} onChange={onChange} className="grid grid-cols-2 gap-4 md:grid-cols-3">
            ...
            {chains
                ...
                .sort((chain) =>
                    isConnected && connectedChain && connectedChain.id == chain.id && !chain.testnet ? -1 : 1
                )
                .map((chain, i) => (
                    <Field key={i} className="flex w-full items-center ">
                        <Radio
                            value={chain}
                            className={clsx([
                                'group relative flex w-full cursor-pointer rounded-lg border border-dapp-blue-400 bg-dapp-blue-400 p-2 text-white shadow-md transition focus:outline-none data-[checked]:border-dapp-cyan-500 data-[checked]:bg-dapp-blue-100 data-[focus]:outline-1 data-[focus]:outline-dapp-blue-50',
                                chain.testnet && 'opacity-30',
                            ])}
                        >
