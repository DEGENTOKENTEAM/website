...
const balanceShares = (buckets: BucketFormParams[], bucketIndex?: number) => {
    ...
    const {
        ...
        restShareOverall,
        unlockedBuckets,
        unlockedBucketsOverall,
    } = buckets.reduce(
        (acc, bucket, i) => ({
            restShareOverall:
                acc.restShareOverall - (bucket.shareLock ? bucket.share : 0),
            unlockedBucketsOverall:
                acc.unlockedBucketsOverall + (bucket.shareLock ? 0 : 1),
            restShare:
                acc.restShare -
                (bucket.shareLock || bucketIndex == i ? bucket.share : 0),
            unlockedBuckets:
                acc.unlockedBuckets +
                (bucket.shareLock || bucketIndex == i ? 0 : 1),
        }),
        {
            restShare: 100,
    ...
    let bucketShare =
        restShare / unlockedBuckets - ((restShare / unlockedBuckets) % STEP)
...
export const BucketsForm = ({
        ...
        balanceShares([
            ...
            ...(existingBuckets || []).map(({ id, duration, burn, share }) => {
                const lockUnit = Object.keys(LockUnits)
                    ...
                    .find(
                        (unit) => !(duration % LockUnits[unit])
                    ) as keyof typeof LockUnits
                return {
                    ...initialBucketData,
                    id,
    ...
    const onAddBucket = () => {
        const updateBuckets = [
            ...cloneDeep(currentBuckets),
            initialBucketData,
        ].map((bucket) => ({ ...bucket, shareLock: false }))
    ...
    useEffect(() => {
        ...
        const unlockedShares = currentBuckets.filter(
            (bucket) => !bucket.shareLock
        ).length
            ...
            unlockedShares > 1 &&
                100 -
                    currentBuckets.reduce(
                        (acc, bucket) =>
                            bucket.shareLock ? acc + bucket.share : acc,
                        0
                    ) >
                    unlockedShares
        )
            ...
            onChange(
                currentBuckets
                    ...
                    .map((bucket) => ({
                        burn: bucket.burn,
                        lock:
                            bucket.lock && !bucket.burn
                                ? bucket.lockPeriod * LockUnits[bucket.lockUnit]
                                : 0,
                        share: bucket.share * 100,
                    })),
                currentBuckets
                    .filter((b) => b.id)
                    .map(({ id, share }) => ({ id: id!, share: share * 100 }))
            )
    }, [currentBuckets])

    const onChangeLockUnit = (
        bucketIndex: number,
        lockUnit: keyof typeof LockUnits
    ) => {
    ...
    const onShareLockClick = (bucketIndex: number) => {
        const updateBuckets = cloneDeep(currentBuckets)
        updateBuckets[bucketIndex].shareLock =
            !updateBuckets[bucketIndex].shareLock
    ...
    const toggleLockStake = (bucketIndex: number) => {
        ...
        if (!updateBuckets[bucketIndex].lock && updateBuckets[bucketIndex].burn)
            updateBuckets[bucketIndex].burn = false
    ...
    const toggleBurnStake = (bucketIndex: number) => {
        ...
        if (!updateBuckets[bucketIndex].burn && updateBuckets[bucketIndex].lock)
            updateBuckets[bucketIndex].lock = false
        ...
        <div className="flex w-full flex-col gap-8">
            <div className="flex flex-col gap-4 rounded-lg bg-dapp-blue-400 p-3">
                <div className="flex items-center justify-between text-lg font-bold">
                    {Boolean(editSharesOnly) ? (
                        <span>Change Pool Shares</span>
                    ) : (
                        <span>Add Pools</span>
                    )}
                </div>
