...
type StartCampaignConfirmationOverlayProps = BaseOverlayProps & {
    onConfirm: () => void
    onCancel: () => void
    error?: any
    isLoading: boolean
...
export const StartCampaignConfirmationOverlay = ({
    isOpen,
    onClose,
    onConfirm,
    onCancel,
    ...
    error,
    campaign,
}: StartCampaignConfirmationOverlayProps) => {
        ...
        <BaseOverlay isOpen={isOpen} closeOnBackdropClick={false} onClose={() => {}}>
            {showSuccessMessage && (
                <div>
                    <div className="flex flex-col items-center gap-6 p-6 text-center text-base">
                        ...
                        <span className="font-bold">Successfully started campaign</span>
                    </div>
                    <Button
                        variant="primary"
                        onClick={onClose}
            ...
            {showContent && (
                <div className="flex flex-col gap-6">
                    <div className="text-3xl font-bold">Start your campaign</div>
                    <div>
                        You&apos;re about to start the campaign{' '}
                        {campaign.config.name ? `"${campaign.config.name}"` : ''}
                        <br />
                        <br />
                        Once you&apos;ve started the campaign, rewards are being distributed over time. The campaign
                        can&apos;t be stopped in favor and protection of the stakers.
                    </div>
